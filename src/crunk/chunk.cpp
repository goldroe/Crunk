...
inline internal bool chunk_is_dirty(Chunk *chunk) {
    return chunk->flags & CHUNK_FLAG_DIRTY;
}
...
inline internal bool block_face_obscured(Chunk *chunk, int x, int y, int z) {
    Block_ID block = chunk->blocks[x + y * CHUNK_DEPTH + z * CHUNK_SIZE];
    Block *basic = get_basic_block(block);
    return !(basic->flags & BLOCK_FLAG_TRANSPARENT);
}

inline internal u8 get_chunk_face_mask(Chunk *chunk, int x, int y, int z) {
    u8 face_mask = 0;
    if (y < CHUNK_HEIGHT-1) face_mask |= FACE_MASK_TOP    *block_face_obscured(chunk, x, y + 1, z);
    if (y > 0)              face_mask |= FACE_MASK_BOTTOM *block_face_obscured(chunk, x, y - 1, z);
    if (z < CHUNK_SIZE-1)   face_mask |= FACE_MASK_NORTH  *block_face_obscured(chunk, x, y, z + 1);
    if (z > 0)              face_mask |= FACE_MASK_SOUTH  *block_face_obscured(chunk, x, y, z - 1);
    if (x > 0)              face_mask |= FACE_MASK_WEST   *block_face_obscured(chunk, x - 1, y, z);
    if (x < CHUNK_SIZE-1)   face_mask |= FACE_MASK_EAST   *block_face_obscured(chunk, x + 1, y, z);
    return face_mask;
}
...
internal void push_opaque_geometry(Chunk *chunk, Auto_Array<u64> &geometry) {
    for (int y = 0; y < CHUNK_HEIGHT; y++) {
        for (int z = 0; z < CHUNK_SIZE; z++) {
            for (int x = 0; x < CHUNK_SIZE; x++) {
                Block_ID *block = block_at(chunk, x, y, z);
                Block *basic_block = get_basic_block(*block);
                u8 l = chunk->light_map[x + y * CHUNK_DEPTH + z * CHUNK_SIZE];

                if (block_is_active(*block) && block_is_opaque(basic_block)) {
...
internal void push_transparent_geometry(Chunk *chunk, Auto_Array<u64> &geometry) {
    for (int y = 0; y < CHUNK_HEIGHT; y++) {
        for (int z = 0; z < CHUNK_SIZE; z++) {
            for (int x = 0; x < CHUNK_SIZE; x++) {
                Block_ID *block = block_at(chunk, x, y, z);
                ...
                if (block_is_active(*block) && (basic_block->flags & BLOCK_FLAG_TRANSPARENT)) {
                    u8 face_mask = get_chunk_face_mask(chunk, x, y, z);
...
internal void light_fill(Arena *arena, Light_Source_List *light_sources, u8 *light_map, u8 x, u8 y, u8 z, u8 level) {
    u8 *light_value = &light_map[x + y * CHUNK_DEPTH + z * CHUNK_SIZE];
...
internal void load_chunk_mesh(Chunk *chunk) {
    ...
    push_opaque_geometry(chunk, chunk->opaque_geo);
    push_transparent_geometry(chunk, chunk->transparent_geo);
